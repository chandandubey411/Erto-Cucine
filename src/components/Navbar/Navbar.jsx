...
export default function Navbar() {
  ...
  const navBg = scrolled
    ...
    : isHome
      ? 'bg-transparent'
      : 'bg-white/95 backdrop-blur-md border-b border-beige-200';

  const textColor = scrolled || !isHome ? 'text-charcoal-800' : 'text-white';
  const logoColor = scrolled || !isHome ? 'text-charcoal-900' : 'text-white';
    ...
    <>
      ...
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${navBg} ${
          scrolled || !isHome ? 'lg:top-0' : 'lg:top-8'
        }`}
      >
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-charcoal-900 text-white overflow-y-auto"
          >
      ...
      <AnimatePresence>
        {searchOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] bg-charcoal-900/95 backdrop-blur-md flex items-center justify-center p-6"
